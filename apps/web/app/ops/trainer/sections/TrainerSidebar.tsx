...
interface TrainerSidebarProps {
  ...
  /** Mobile/tablet off-canvas state — ignored at md+ where the sidebar is always visible. */
  open?: boolean;
  onClose?: () => void;
}
...
export function TrainerSidebar({ activeView, onSwitchView, badges = {}, open = false, onClose }: TrainerSidebarProps) {
  return (
    <>
      ...
      >
        ...
        <div style={{ borderBottom: "1px solid var(--border)" }} className="px-3.5 py-2.5 flex items-center gap-2">
          ...
          <button onClick={onClose} className="ml-auto w-6 h-6 flex items-center justify-center rounded md:hidden" style={{ color: "var(--text3)" }} aria-label="Close menu">
            ✕
          </button>
        </div>
