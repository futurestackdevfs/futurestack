...
const NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: "👥", label: "Leads", view: "leads", badgeKey: "leads", badgeColor: { color: "var(--blue)", bg: "var(--blue-d)" } },
      { icon: "🎓", label: "Students", view: "students" },
      { icon: "📅", label: "Batches", view: "batches" },
    ],
  },
  {
    group: "Oversight",
    items: [
      { icon: "🧑‍🏫", label: "Trainer Activity", view: "trainers" },
      { icon: "🚩", label: "Escalations", view: "escalations", badgeKey: "escalations", badgeColor: { color: "var(--red)", bg: "var(--red-d)" } },
    ],
  },
];

export function CoordinatorSidebar({ activeView, onSwitchView, badges = {}, open = false, onClose }: CoordinatorSidebarProps) {
  return (
    <>
      {open && (
        <div
    ...
    >
      <div style={{ borderBottom: "1px solid var(--border)" }} className="px-3.5 py-2.5 flex items-center gap-2">
        <div className="w-6 h-6 rounded flex items-center justify-center text-[12px] shrink-0" style={{ background: "var(--blue-d)", color: "var(--blue)" }}>🗂</div>
        <div>
          <div className="text-[10.8px] font-semibold" style={{ color: "var(--text)" }}>Coordinator Console</div>
          <div className="font-mono text-[8.5px]" style={{ color: "var(--text3)" }}>role::coordinator</div>
        </div>
            ...
            return (
              <div key={item.view}
                ...
                style={{
                  ...
                  background: isActive ? "var(--blue-d)" : "transparent",
