...
export function SalesTopbar({ user, currentView, onSearch, onNewSale, onSignOut, onMenuClick }: SalesTopbarProps) {
  ...
  return (
    ...
    >
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <button
          onClick={onNewSale}
          className="flex items-center gap-1.5 font-mono text-[9.5px] font-bold px-2.5 py-1 rounded cursor-pointer whitespace-nowrap"
          style={{ background: "var(--orange)", color: "#fff", border: "1px solid var(--orange)" }}
          onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.opacity = "0.85"; }}
          onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.opacity = "1"; }}
        >
          <span style={{ fontSize: 11 }}>＋</span><span className="hidden sm:inline"> New Sale</span>
        </button>
        ...
        <div className="relative" ref={profileRef}>
          ...
          >
            <span className="w-6 h-6 rounded-full flex items-center justify-center text-[9.5px] font-bold" style={{ background: "var(--orange-d)", color: "var(--orange)" }}>
              {user.initials}
            </span>
          ...
          {profileOpen && (
            ...
            >
              <div className="px-3 py-2.5" style={{ borderBottom: "1px solid var(--border)", background: "var(--panel)" }}>
                ...
                <div className="font-mono text-[9px] mt-0.5" style={{ color: "var(--orange)" }}>role :: {user.role.toLowerCase()}</div>
              </div>
              <button
