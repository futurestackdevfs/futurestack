...
export class CertificatesService {
  ...
  async checkAndIssueCertificate(studentId: string, courseId: string): Promise<{ issued: boolean; credentialId?: string }> {
    ...
    const rank = await this.prisma.certificate.count({ where: { courseId } }) + 1;
